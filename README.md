...
|---|---|---|
| `scripts/dicom_to_nifti.py` | **Single-subject test tool.** Converts one DICOM subject folder to NIfTI. Useful for inspecting an individual scan; not used for batch conversion. | `SimpleITK`, `pydicom`, `PyYAML` |
...
| `scripts/explore_amgen_bboxes.py` | **Read-only XIF/AMIDE inspector.** Probes the Amgen SUV data for per-animal bounding boxes in `.xif` files. Confirmed result: all 137 XIF files are raw binary float32 arrays with no bbox/ROI metadata — CT auto-segmentation is used instead. Run with `--summary` or `--scan <id> --verbose`. | `PyYAML` |
...
**Stage 2 — Animal segmentation:**
1. Downsample CT to ~1 mm/voxel isotropic
2. Apply bone threshold (≥ 300 HU) — scanner bed carbon-fibre/foam stays below threshold
3. Binary closing with 3 mm radius to bridge intra-mouse bone gaps (ribs, vertebrae)
4. Connected-component analysis; keep top-N components by volume (N = expected mouse count from manifest)
5. Map component bounding boxes back to physical mm coordinates (used by Stage 3 for both CT and PET)
6. Sort animals by grid position (Y ascending, then X ascending) to assign consistent positional IDs

**Stage 3 — Per-mouse crop:**
1. Map physical bbox to each image's voxel space via `TransformPhysicalPointToIndex` (handles different CT/PET spacings and origins correctly)
2. Apply 20 mm padding around each bone bbox to capture full soft tissue
3. Write cropped volumes to `mice/{cohort}_{genotype}_{num:02d}/week_{N}/ct_hi.nii.gz` and `pet_*.nii.gz`
